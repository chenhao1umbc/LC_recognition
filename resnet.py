...
for epoch in range(201):
    ...
    if acc_all[-1] == max(acc_all):
        torch.save(model, './res/resnet/best_resnet_aug.pt')

    if epoch > 50 and epoch%20 == 0:
        plt.plot(tr_loss[-50:], '-o')
        plt.plot(val_loss[-50:], '--^')
        plt.legend(['Training', 'Validation'])
        plt.savefig(f'./res/resnet/resnet_loss_{epoch}_aug.png')
        plt.close('all')

torch.save([tr_loss, val_loss], './res/resnet/resnet_tr_val_loss_aug.pt')
torch.save(acc_all, './res/resnet/val_acc_aug.pt')
plt.figure()
plt.plot(tr_loss, '-o')
plt.plot(val_loss, '--^')
plt.legend(['Training', 'Validation'])
plt.savefig('./res/resnet/resnet_loss_func_values_aug.png')
plt.close('all')
print('done')